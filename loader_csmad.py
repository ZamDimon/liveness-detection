...
def get_file_type(path): 
    """
    get_file_type returns a file extension given a path
    """
    base = os.path.basename(path)
    return os.path.splitext(base)[1] # e.g., .png, .jpg
    ...
    if balance_dataset and undersampling==False:
        ...
        for j in range(0, len(images_per_class)):  #[[4k], [4k], [4k], [4k]]
            if images_per_class[j] < _max:
                ...
                offline_generator = tf.keras.preprocessing.image.ImageDataGenerator(rotation_range=20, zoom_range=0.15, width_shift_range=0.2, height_shift_range=0.2, shear_range=0.15, horizontal_flip=True, fill_mode="constant")
